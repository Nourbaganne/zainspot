...
const Page = ({ params }: { params: { id: number } }) => {
    ...
    const { data, isLoading, isError, error } = useQuery({
        ...
        queryFn: () => axiosInstance.get(`/user/${params.id}`, {
            headers: {
    ...
    const BUSINESS_LIST = [
        ...
        { title: 'Location', value: `${currentUser?.city}, ${currentUser?.state}, ${currentUser?.country}` },
        { title: 'Future Regions of Interest for expansion', value: currentUser?.interestRegion },
    ...
    return (
        <div className='flex flex-col gap-6 bg-background-foreground md:px-24 md:py-8 md:pb-20' >
            ...
            <div className='card flex flex-col gap-10 md:py-10'>
                <div className='flex justify-between'>
                    <div className='flex flex-col gap-4'>
                        <div className='flex gap-2'>
                            ...
                            <select name="" id="" className='bg-background-foreground rounded-md border-border'>
                                <option value={data?.data?.role.role}>{data?.data?.role.role}</option>
                            </select>
                        </div>
                        <div className='flex flex-col gap-2'>
                            <p className='text-sm text-span font-light'>Member since {new Date(data?.data.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                            <div className='flex gap-2 items-center text-sm'>
                                ...
                                <div className='flex gap-1 items-center' >
                                    ...
                                    <p>{new Date(data?.data.birthday).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <SubscriptionList userId={params?.id} access_token={user?.access_token} />
