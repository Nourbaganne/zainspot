...
const Users = () => {
  ...
  const { roles, isLoading: rolesLoading, isError: rolesError, error: rolesErrorMsg } = useRoles();
  ...
  // Handle roles loading and error
  if (rolesLoading) {
    return <Loader />;
  }

  if (rolesError) {
    return <h1>Failed to load roles: {rolesErrorMsg.message}</h1>;
  }
            ...
            <>
              ...
              <div className='mt-6 flex justify-end items-center gap-4'>
                ...
                <button
                  ...
                  onClick={() =>
                    setCurrentPage((prev) =>
                      Math.min(prev + 1, data?.data.totalPages || prev)
                    )
                  }
                >
                  Next
