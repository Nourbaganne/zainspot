...
import eyeOutline from '../assets/register/eye-outline.svg';
import eyeOffOutline from '../assets/register/eye-off-outline.svg';
import checked from '../assets/register/checked.svg';
import close from '../assets/register/close-outline.svg';
...
const InputPassword = ({
	...
	const PASSWORD_REQUIREMENT_MENU = [
		...
		{ title: '1 number', isCorrect: passwordRequirements.number },
		{
			...
			isCorrect: passwordRequirements.specialChar,
		},
	];
		...
		<div ref={inputRef} className='flex flex-col gap-2 w-full relative'>
			...
			{isTypingPassword && errors && (
				<div className='relative'>
					<h1
						className={`pl-4 text-alert ${
							name === 'password' && 'cursor-pointer'
						} `}
						onClick={() => setCheckPasswordReq(!checkPasswordReq)}
					>
						{errors}
					</h1>
						...
						<div className='absolute bg-background z-30 shadow-xl p-4 border rounded-md'>
							{PASSWORD_REQUIREMENT_MENU.map((item, index) => (
								<div className='flex gap-2' key={index}>
									{item.isCorrect ? (
										<Image src={checked} alt={`${index}`} />
									) : (
										<Image src={close} alt={`${index}`} />
									)}
									<h1
										className={`${
											item.isCorrect ? 'text-primary' : 'text-alert'
										}`}
									>
										{item.title}
									</h1>
