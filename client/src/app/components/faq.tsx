...
export default function FaqPage() {
	...
	const [stepsWithTranslations, setStepsWithTranslations] = useState<Steps[]>([]);

	useEffect(() => {
		// Preload the translated steps
		const translatedSteps = originalSteps.map((step) => ({
			...step,
			title: t(step.title),
			body: step.body.map((faq) => ({
				question: t(faq.question),
				answer: t(faq.answer),
			})),
		}));

		setStepsWithTranslations(translatedSteps);
	}, []);
	...
	function handleSearchTextChange(text: string) {
		...
		setSteps(
			stepsWithTranslations.filter((step) => {
				// Access the translated title and compare
				const titleText = step.title.toLowerCase();
				const titleMatch = titleText.includes(searchTextLower);

				// Check the translated question and answer for a match
