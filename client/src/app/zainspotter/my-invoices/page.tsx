...
const Page = () => {
    ...
    const { data, isLoading, error, isError } = useQuery({
        queryKey: ['invoices'],
        queryFn: () => axiosInstance.get(`/invoices/${user?.user.userId}`, {
            headers: {
                            ...
                            <div className='min-w-max'>
                                ...
                                <div className='max-h-[340px] overflow-y-auto'>
                                    {data?.data.map((invoice: Invoice, index: number) => (
                                        <div className='flex gap-3 md:gap-0 items-center border-b text-text font-light pl-4  py-5' key={index}>
                                            <ul key={index} className='w-full grid grid-cols-11 items-center'>
                                                ...
                                                <li className={`col-span-2 pl-4 flex items-center gap-2 ${invoice.status === 'Paid' ? 'text-primary' : invoice.status === 'Unpaid' ? 'text-yellow-500' : 'text-alert'}`}>
                                                    <span className={`w-3 h-3 rounded-full ${invoice.status === 'Paid' ? 'bg-primary' : invoice.status === 'Unpaid' ? 'bg-yellow-500' : 'bg-alert'}`}></span>
                                                    {invoice.status}
                                                </li>
