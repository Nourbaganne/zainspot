...
import { useState, useRef, useEffect, MouseEvent as ReactMouseEvent, useContext } from 'react';
...
const Page = () => {
    ...
    const { data, isError, error, isLoading } = useQuery({
        ...
        queryFn: () => axiosInstance.get(`/payment-history/${user?.user?.userId}`, {
            headers: {
                            ...
                            <div className='min-w-max'>
                                ...
                                <div className='max-h-[340px] overflow-y-auto'>
                                    {data?.data.map((payment: PaymentHistory, index: number) => (
                                        <div className='flex gap-3 md:gap-0 items-center border-b text-text font-light pl-4 py-[14px]' key={index}>
                                            <ul className='w-full grid grid-cols-11 items-center'>
                                                <li className='col-span-3'>
                                                    <div className='flex flex-col text-sm'>
                                                        <h1 className='font-normal'>{payment.subscription.country}</h1>
                                                        <span className='text-span'>{payment.subscription.type}</span>
                                                    </div>
                                                </li>
                                                ...
                                                <li className={`col-span-2 pl-4 flex items-center gap-2 ${payment.status === 'Complete' ? 'text-primary' : payment.status === 'Pending' ? 'text-yellow-500' : 'text-alert'}`}>
                                                    <span className={`w-3 h-3 rounded-full ${payment.status === 'Complete' ? 'bg-primary' : payment.status === 'Pending' ? 'bg-yellow-500' : 'bg-alert'}`}></span>
                                                    {payment.status}
                                                </li>
                                            </ul>
                                            <div ref={menuRef}>
