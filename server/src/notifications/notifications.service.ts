...
export class NotificationsService {
    ...
    async sendNewNotification(object: string, notificationType: string) {
        try {
            const validNotificationTypes = [
                'newCityNotif', 'specialOfferNotif', 'marketingNotif',
                'birthdayNotif', 'greetingsNotif', 'monthlyPaymentAlert',
                'paymentReceipt', 'paymentFailure', 'imminentPayment',
                'paymentExpired', 'passwordReset', 'accModifications',
            ...
            await Promise.all(userEmails.map(email => this.sendNotifMail(email, object)))
    ...
    async sendNotifMail(email: string, object: string) {
        try {
            const html = `
